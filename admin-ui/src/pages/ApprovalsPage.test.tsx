...
describe("ApprovalsPage", () => {
  ...
  it("requires a reason and sends an approval decision", async () => {
    const fetchMock = vi.fn<typeof fetch>((input, init) => {
      ...
      if (url.includes("/decision")) {
        ...
        return Promise.resolve(
          jsonResponse({ proposal, approval: null, execution: { status: "succeeded" } }),
        );
      }
    ...
    expect(screen.getByRole("status")).toHaveTextContent("Add a decision reason");
  ...
  it("submits a rejection and surfaces failed API mutations", async () => {
    ...
    const fetchMock = vi.fn<typeof fetch>((input, init) => {
      const url = requestUrl(input);
      if (url.includes("action-proposals")) {
        return Promise.resolve(jsonResponse({ items: [proposal], total: 1, limit: 100, offset: 0 }));
      }
      if (url.includes("approvals?")) {
        return Promise.resolve(jsonResponse({ items: [], total: 0, limit: 100, offset: 0 }));
      }
      if (url.includes("/decision")) {
        ...
        if (failDecision) {
          return Promise.resolve(new Response(JSON.stringify({ detail: "Proposal became stale" }), {
            status: 409,
            headers: { "Content-Type": "application/json" },
          }));
        }
        return Promise.resolve(jsonResponse({ proposal: { ...proposal, status: "rejected" }, approval: null, execution: null }));
      }
      return Promise.resolve(jsonResponse({}));
    });
    vi.stubGlobal("fetch", fetchMock);
    ...
    expect(await screen.findByRole("status")).toHaveTextContent("Proposal rejected");

    failDecision = true;
    fireEvent.click(screen.getByRole("button", { name: "Reject" }));
    expect(await screen.findByRole("status")).toHaveTextContent("Proposal became stale");
  });
  ...
  it("renders live proposals as advisory acknowledgements without execution controls", async () => {
    const liveProposal = {
      ...
      manual_action_instructions: ["Review the recommendation manually in Ads Manager."],
    };
    const fetchMock = vi.fn<typeof fetch>((input) => {
      const url = requestUrl(input);
      if (url.includes("action-proposals")) {
        return Promise.resolve(
          jsonResponse({ items: [liveProposal], total: 1, limit: 100, offset: 0 }),
        );
      }
      return Promise.resolve(
    ...
    expect(await screen.findByText("LIVE META — READ-ONLY ADVISORY")).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Acknowledge advisory" })).toBeEnabled();
    expect(screen.queryByRole("button", { name: "Approve" })).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: /execute/i })).not.toBeInTheDocument();
    expect(screen.getByLabelText("Select decrease_budget proposal")).toBeDisabled();
  });
});
