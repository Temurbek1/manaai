...
describe("RunsPage", () => {
  ...
  it("filters, paginates, renders failures, and redacts untrusted timeline details", async () => {
    const fetchMock = vi.fn<typeof fetch>((input) => {
      ...
      return response({ items: [run], total: 21, limit: 20, offset: url.includes("offset=20") ? 20 : 0 });
    });
    vi.stubGlobal("fetch", fetchMock);
    const rendered = renderWithSession(<RunsPage />, "viewer");

    expect(screen.queryByText("Provider applied a different value")).not.toBeInTheDocument();
    fireEvent.click(await screen.findByRole("button", { name: /run-0000/ }));
    await waitFor(() => expect(rendered.container).toHaveTextContent("Provider applied a different value"));
    ...
    fireEvent.change(screen.getByLabelText("Filter runs by status"), { target: { value: "failed" } });
