...
function mockAgentApi(
  ...
  return jest.fn<typeof fetch>((input, init) => {
    ...
    if (url.includes("/agents/marketing-agent") && !url.includes("/run"))
...
describe("AgentsPage", () => {
  ...
  it("disables configuration, schedule, status, and kill-switch controls for viewers", async () => {
    ...
    fireEvent.click(
      await screen.findByRole("button", { name: /Marketing Agent/ }),
    );
  ...
  it("requires confirmation before an admin changes the per-agent kill switch", async () => {
    const fetchMock = mockAgentApi((url, init) => {
      if (url.includes("/kill-switch/agents/marketing-agent")) {
        expect(init?.method).toBe("PUT");
    ...
    expect(fetchMock).not.toHaveBeenCalledWith(
      expect.stringContaining("/kill-switch/agents/marketing-agent"),
      expect.objectContaining({ method: "PUT" }),
    );
    fireEvent.click(
