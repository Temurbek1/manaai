...
The canonical direction and current Growth implementation for MANA OPERATION AI are documented in
...
compatibility-preserving `marketing-agent` alias inside Growth & Conversion.
...
deploy/nginx/               Production reverse-proxy examples
...
The full-stack production topology uses two hostnames:

- `ai.360rec.uz` proxies the public API on port 8000 using
  `deploy/nginx/ai.360rec.uz.conf`;
- `ai-frontend.360rec.uz` proxies the Next.js admin on port 3000 using
  `deploy/nginx/ai-frontend.360rec.uz.conf`.

The browser calls only the admin hostname. Next.js proxies `/api/*` to the private
`ADMIN_FASTAPI_BASE_URL` (normally `http://api:8000`). Install both Nginx examples under
`sites-available`, enable them under `sites-enabled`, configure DNS, and issue an independent TLS
certificate for each hostname. Full deployment and secret requirements are in
[`README.operation-ai.md`](README.operation-ai.md) and
...
make test          # pytest, excludes live provider checks, plus admin-ui jest
make verify        # everything above, plus admin build and npm audit
make audit-verify  # migrations, mutation/security audits, browser E2E, and Docker build
```

Tests never call the real OpenAI API and never call Meta write APIs. Live provider checks are
