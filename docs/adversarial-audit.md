...
| API and admin UI responses lacked a consistent browser security-header policy. | Clickjacking, content sniffing, or looser script execution after a separate injection defect. | **Fixed:** API headers plus Next.js CSP/frame/content/referrer/permissions policy; header and standalone production checks pass. | CSP must be retested if external assets or inline scripts are introduced. |
